...
const allNumber = (...chars: string[]): boolean => chars.map(v => isNumber(v)).reduce((prev, curr) => prev && curr)
const resolveEquation = (givenOperands: string[]): string[] => {
    ...
    let operands = isNumber(givenOperands[0]) ? ['+', ...givenOperands] : givenOperands;
    operands = operands.length % 2 === 0 ? operands : operands.slice(-1);
    let operators = operands.filter(op => !isNumber(op));
    operands = operands.filter(op => isNumber(op));
    let acc = operands
                .map((op, i) => operators[i] === '+' ? Number(op) : Number(-op))
